...
放拜年很多。这里把_abs_start以地址绝对数的方式置入源码，然后用ld加载，是为了节省指令数量。
...
像stm32，就是通过外部的两个boot引脚决定的。这样就导致我们的程序按真实内存块映射的地址编译，
运行、调试的时候却在映射的地址运行。比如stm32，它把32位的闪存地址0x08000000映射到低地址位0x00000000，
调试的时候，代码都在0x00xxxxxx上运行，调试器不知道它和0x08xxxxxx是等效的，符号表对应不上，
调试器就无法读取每个符号的值，调试就非常困难，所以生成一段代码，无论开始运行的地址如何，
都在一开始跳转到0x08xxxxxx段的地址上，随后都是和符号表能对应的符号了。这里的内存块映射由硬件设计完成；
回到我们操作系统上，从虚拟地址到物理地址的线性映射和内存块的固定映射非常相似，就可以参考已有的设计。
