...
这部分的处理是链接器完成的。但是如果我们没定义`gp`，它的值就是任意的，在模拟器里值可能是零。
...
计算每个Hart的栈地址需要使用乘法。有M的核好说，没有M指令集的核上就用循环加法代替，Hart的数量也不会多，
占用的时间不会很长。
...
而是清除自己的软件中断标记，再等待第0个核调用软件中断，让它启动。
...
这里给出简单的`mp_hook`实现。需要调用`riscv-sbi`的函数，清空自己的软件中断标记。当第0核完成main函数的启动步骤后，
它需要这样运行：
...
上面文档就有了（别的寄存器就是这样设计的）。下次需要修改`riscv`这个包的时候，顺便完成这项小工作。
