...
这里不建议使用全局属性宏，一方面是还没稳定，另一方面是如果稳定，目前Rust的实现要解析所有的代码，解析量也是巨大，可能造成编译较慢。
...
听教授说以前有同学把跑在M级的Linux移植到K210上，也是相当厉害，相关的思路可以稍作借鉴。
...
这样如果想要适配系统到新的硬件，只需要自己编写兼容代码，在硬件配置上非常灵活，而且工作量不大。
...
稍微提一下这个问题的一些想法。bsp是板级支持库的缩写。传统上我们开发嵌入式裸机程序，
我们需要找到bsp，然后调用里面的函数。如果在有rtos的情况下，bsp是为每个操作系统对应每个板实现的，
移植操作系统就需要自己制作新的bsp，复制文件，非常费时费力。
Rust做得比较好的地方就是模块化开发，它把原来bsp的内容全部拆分，设备驱动的部分拆为单独的hal库，
地址表拆分成pac库。这样如果你想做传统意义上的bsp，你会发现只是几个hal库的简单包装，
还不如拆开来分别用每个hal库。因此，Rust裸机的嵌入式开发就只需要组合使用几个hal库即可，无需使用bsp。

当然会问了，这还是没有os的情况下，如果有rtos呢？目前传统意义的的rtos设计都是先到芯片，
然后如果做得好是分别为板实现的，这些实现就是bsp的一些思想。
会发现传统意义上的bsp其实包含几个部分：硬件的驱动，M层入口和中断代码，S层入口和中断代码，
还有S层具体的操作系统实现。你会发现很多情况下设备树这种东西甚至是作为单独文件提供的。
本次项目综合运用M运行时、S运行时两层最小运行时，然后灵活配置SBI实现和操作系统实现，
这样最大化地利用Rust模块化和灵活性的特点。你会发现传统的bsp已经被拆掉了，我们拆成了以上几个部分。
这样开发者只需要写非常简单的胶水代码，就能快速配置到新的系统。
未来我们从事Rust内核的开发，包括嵌入式的RTOS内核，就不需要再单独提到bsp的概念了。

### 3. 实现SBI实现库遇到的问题

就是SBI实现库的输入可能有各种各样的类型，要抽象成同一个虚函数表。
因为Rust没有类似于Java接口的概念，所有动态类型的东西要么用Any，要么用dyn Trait。
后者的生命周期非常玄学，需要花大量时间学习，在单独的工作空间里探索，得到最好的设计。
今天大量的时间都花在这个地方了。希望明天能给出较好的设计。

我觉得未来写操作系统的设备树会遇到相同的问题。设备树也是各种各样的类型，要存到一个表里面，
运行的时候还要分辨出是哪个类型的设备。`uefi-rs`里有一个guid宏的设计比较好，
未来写设备树可以参考这个设计。SBI实现库用这个就过于重量级了，还是用dyn Trait吧（如果可能）。

## 第20天

### 1. 启动SBI实现

我的rustsbi实现现在可以启动了。不过还有一个很奇怪的问题，到某个阶段，栈里的一些内容会变成0，
导致串口输出找不到正确的位置。暂时不调试，先把中断处理器写完，然后再考虑这些问题。
也许写完后，和riscv-rt团队的朋友聊一聊，看看怎么把操作系统的这一块兼容进去。

另外如果没有陷入处理器的情况下，如果程序停在0x00000000的位置，你要考虑到可能是发生了异常，
因为mtvec的值还是零，将会用默认的模式，跳转到0x00000000位置的陷入处理器，
然而这里的内存通常并没有办法访问，就会又出现读异常，又会跳到这个位置，导致陷入循环。
需要意识到这一点，可以先写完陷入处理器，再仔细排查原因。

于是暂时是可以在qemu启动了：

```text
   Compiling supervisor-app v0.1.0 (two-level-os-lab/supervisor-app)
...
今天处理了陷入处理函数。关于设备树部分还要再想，M到S的调用是没问题了。后期改一改medeleg这些就能让操作系统处理异常了。
还需要做多核启动部分。

其实中断处理的部分还能改，这部分甚至可以做到riscv-rt里面。需要明确s0-s11这些寄存器的用途。
如果中断处理函数不再调用其它函数，这些是不用保存的。什么时候需要保存，以前看到过一篇文章特别好，可惜链接找不到了。
回头再搜。

riscv-rt里面其实就可以做准备到其它特权等级的入口了。这部分可以做成feature，等设计稳定之后，和社区的朋友商量怎么制作。

29天前我关于看门狗抽象的部分给embedded-hal项目组做了提案，今天得知被通过了，我做了一些文档的小修改，然后
非常高兴我的技术可以贡献到社区里。
...
这个位存在于mstatus寄存器的第17位，在32位、64位系统下都是有效的。这个位能决定各个模式下读取、写入类指令的访问方式。
sstatus寄存器的第17位是WPRI位。

MPRV位的第一个M是修改的意思，不是机器特权级的意思。
当这个位为1时，读或者写内存的操作将会修改使用的特权级设置，比如将总会通过页表转换和保护，而且还会使用对应特权级的大小端库设置。因为SBE、UBE这些位是可以设置对应特权级大小端序的。
如果这个位为0，读写内存的操作特权级设置是不变的，也就是说使用当前的特权级设置。
还有一个特殊的规则是使用mret返回更低的特权级时，MPRV会被重置为0。

这样机器特权级就可以读写虚拟地址了。当拿到虚拟地址，就设置这个位为1，然后执行读取指令，然后设置回0就可以了。
比如操作系统调用ecall时，SBI环境拿到的是虚拟地址，要访问这个虚拟地址，与其用软件解析页表和端序，不如使用这种方便的方法。

标准里好像没细说是使用哪种特权级，只说明一、通过页表转换和保护寄存器，二、使用对应特权级的大小端序。

### 2. 爆栈

我一开始给每个核设置的M层栈是两个KB，最终发现一些诡异的情况：

- mhartid为1的这个核总是没法启动，无论我虚拟多少个核
- 原本设置的锁失效了，比如我明明设计了锁，标准输出还是乱的，就好像我没锁一样

最终排查问题，是发现我使用第0个核完成初始化操作。猜测这个第0核可能写入了超过两个KB的栈内存，
覆盖了汇编里为第1核准备的栈，导致第1核启动失败。

把每个核的栈扩大到128KB，问题解决。

还需要进一步验证这个结果。这两个诡异的情况和核的数量无关，反而和栈大小有关。内核的爆栈很难被检测到，
我怀疑后续可以通过pmp解决这个问题，每个核只允许访问自己的栈，否则出现异常。
pmp部分做完之后，再回头优化这个问题，检测爆栈就轻松很多。

### 3. 如何配置qemu虚拟多个核

这个在前面编写操作系统的部分已经涉及到了：
...
这几天一直没注意记录，做的更改很多，很细。学习了新的asm语法，然后向后推进更多的SBI实现。

目前同一个对象分两个环境用的还有点难做，比如CLINT可以是IPI来源，也可以是mtime来源。然后mtime方面还需要
在非法指令异常里模拟指令。这个可以在硬件中间层库里慢慢细化着做会好一点，比如IPI的CLINT和mtime的CLINT分开来。

今天和老师同学们交流，了解了RISC-V指令集最新的实践。我国的相关实验室和单位进展还是非常快的。
据我在社区了解，大量的芯片厂已经流片了最新的RISC-V设计，不过可能很大一部分是第一次设计此类芯片，
可能存在设计漏洞，这就不适合把文档第一时间公开。GD32V的厂家有很足的设计arm芯片的经验，他们能把文档第一时间公开。
未来，文档齐全的RISC-V芯片产品会越来越多。

明天我要参加probe-rs团队的“Oxidize impl days”活动，用一天时间完善probe-rs的设计。
我挑选了GD32V支持的任务。明天操作系统这里我停工一天，集中精力把社区的调试器支持制作完整。

## 第26天

### 1. 我去参加了社区活动

今天参加了社区的“铁锈之日”活动。活动开始安排了线上直播，讲解社区还需要集中攻关的内容，然后活动时间两天，
每个人可以挑选自己想完成的任务，在活动期间内能获得社区最大的帮助。
刚开始的线上讲解在UTC早上十点（北京时间下午18点），不过我挺可惜刚好在另一个地方开会，没有来得及看讲解。
不过活动给出了详细的网页，列出了社区近期攻关的目标，大概包括很多生态链的工具要开发，还有包括蓝牙栈啊这些共同维护的实现需要做的努力。

我挑选了daplink上实现jtag调试的任务，不过做了一整天，发现参考源码非常少，arm的文档历史包袱很重，
我最终只完成了最底层包通讯的部分，上面的逻辑还需要后面的开发者继续完成。可能和原作者的观点类似，这个情况出现非常少，
很多daplink的调试器本身只用swd协议，只有riscv的板载调试器还在酝酿，暂时只有arm的调试器能用的情况下，这个才是必需的，
否则需求非常少，很难有应用领域。

这次活动我发现社区的调试软件支持已经有一定进展，尤其是probe-rs这块的生态，进展相当迅速，已经支持大量品牌的芯片。
他的原理大概就是代替了openocd，抛弃了旧架构的很多缺陷（比如只能同时调试一个设备），是Rust社区里进步的地方。
不过我从个人的观点来看，他们的调试软件还缺少中间一层的抽象，要添加新的芯片只能修改代码，这就不容易使用，
可能未来软件的架构成熟后，插件系统这些就可以开始做了。他们网站上的目标是使用WebAssembly做插件系统。

铁锈之日活动我还阅读了社区提供的rtic架构。他们这个架构是发了论文的，是适配大多数嵌入式设备，基于中断和时钟的资源配置和调度架构。
只要具体的指令集架构（ISA）能给定一个中断控制器和一个系统时钟，就可以以这些为调度器，制作非常精致的任务系统。
他们的架构合理使用了Rust的过程宏，语法尤其丰富，甚至专门写了个库叫做rtic-syntax，用来解析rtic架构的过程宏语法。
就相当于非常合理地运用Rust的语法，包括过程宏，还有所有权语义，完成裸机环境的软件设计，避免不同核之间、核心代码和中断代码之间的资源竞争。
目前他们的架构在cortex-m上有很好的支持，不过在riscv上支持还不多。
个人分析原因可能是riscv的中断控制器是根据应用不同自行适配的，可能不会给定统一的标准，这样rtic就需要有更多的中间层，
去支持用户自己提供的中断控制器。riscv-rt运行时库是已经支持这一块功能了。

### 2. 一些启示

今天参加这些活动，我发现软件上社区的实现优化非常快，我们假如有配套硬件，可能可以共同发展这一块的生态。

未来我们操作系统课如果有课本配套硬件，建议安装板载调试器。暂时可以有基于FTDI（FT2232）的调试器，比如sipeed就有很多款调试器非常合适。
不是只安装一个转串口的芯片，而是安装完整的调试设备，这样就很能满足操作系统开发的需求。
强烈建议此类调试设备可以直接设计到板上，作为学习设备，就不需要考虑调试过程中电气连接这类很杂的问题，专心地学习操作系统。

rtic架构可能也能给制作裸机系统提供一些启示。对于我们学习来说，这些方便的架构，都是需要我们完整了解操作系统开发的基础知识，
然后才为了简化开发流程再使用的。在完整了解知识，而且能够编写自己的操作系统之后，在编写后续其它的系统，快速验证一些功能和设计，
才需要考虑使用包装更好的架构。包括我可能在设计的rustsbi也属于这一类目的：在充分了解有关知识后，要在其它新的芯片上快速验证，
就可以使用这样包装比较好的依赖库。如果还属于初学者，第一次不应该直接接触这些库，而是先按部就班学完基础知识，熟练后才应当考虑。

### 3. M层运行时的一些思路

riscv-rt运行时目前的设计还不会保留内存空间。可能我需要小幅度修改设计，把所有内存区段的结构做小调整，以便有能力为操作系统预留空间，
这样就可以只烧录这一部分，留下的空间烧录操作系统了。

目前的设计还是：数据区、只读数据区、零初始化区（bss，可能有更好的翻译）和代码区长度固定，堆区长度固定，而剩余的长度全部归入栈区。
预计需要调整的是剩余长度的分配，栈区长度从Hart数乘以可配置的每核栈长度得到，然后堆区的长度也是可配置而且固定的。
这样剩余的长度就全部空出来了，就作为保留使用，操作系统就可以往这里写。

还需要验证指令和数据存在不同的内存块时，这个设计是否仍然有效。举一个例子，cortex-m-rt的设计是：
代码区、只读数据区放在闪存里，数据区、bss区放在可读写内存RAM里面，剩下的全都是堆内存；
栈内存直接从RAM的底部开始向上生长。而堆、栈内存的长度是代码决定的，链接器脚本不指定。
riscv-rt的设计可以做一些调整，因为它不像cortex-m-rt，cortex-m有几块内存、每个物理地址干什么是有规定的，而riscv不规定，
所以堆区、栈区的设计可以保留，长度可以是由实现配置而且给定的，以便后面灵活的内存配置。然后内存块的截止地址应该由sbi实现决定，通常它将跳转到这个地址，
riscv-rt可能不负责怎么给这一块地址，而是留给sbi实现。

后续时间慢慢做这一项工作。

### 4. 修改sbi运行时设计的一些思路

需要调整操作系统初始页表的设计。目前还是写死的，可能不同设备的物理地址不同。感谢王润基学长的灵感，可以基于入口的pc值
设计初始页表。这样就可以烧录在任何位置了。入口的pc可能需要是对齐的，不过这个不要紧，链接器脚本会完成。

另外还有一点就是启动栈是可以回收的。有些操作系统的设计会回收，而有些不会，需要考虑这一点。

还有一个就是操作系统如果想直接烧录，生成与位置无关的代码是很重要的。操作系统可以朝这个方向编译。
其实一开始的启动代码考虑这些就可以了。后面都是在虚拟地址上运行，只要启动代码里写好了页表，就没有问题。
需要注意的是启动代码的页表和目标的物理地址是有关系的。这里可以根据pc的值生成页表，这样第一条指令的pc要对齐到4K，就可以生成页表了。

非常重要的一条指令就是`auipc`。比如`auipc t0, 0`，就可以把`auipc`指令所在的pc地址传入t0里面。
这项工作明天就可以开始尝试了。

## 第27天

### 1. 地址无关的操作系统内核（SBI运行时部分）

今天把SBI运行时的一些思路做完了，原型在[这里](https://github.com/luojia65/pcrel-os-lab)。

做的时候发现一个坑就是，我以前认为Sv39的页表项是这样的：

| 54..=63 | 10..=53 | 0..=9 |
|:-------:|:-------:|:-------:|
| 保留 | 物理页号，也叫PPN | 一大堆标签 |

其实物理页号也是分为三段的，分别是PPN[2]、PPN[1]和PPN[0]。而且访问页表的时候也要注意每段物理页号对应的对齐。
Sv39页表有三级，如果当前的级别数以下对应的PPN值都不是零，那就属于非对齐的超页，将产生页错误异常。
比如说我翻译地址，第一步在访问第2级虚拟地址的1G巨页，页表项里的值不是指向下一级页表的（RWX不为0b000），
那么页表项里PPN[1]和PPN[0]都必须是0，但是PPN[2]可以是程序给定的。
又比如第二部访问第1级虚拟地址，是2M的大页，这样页表项的值中PPN[0]必须是0，PPN[2]到PPN[1]都是可以设定的。

第一次做的时候，以为整个PPN写入都是正确的，我直接把对齐到1G的虚拟地址创建巨页，对应的项不是指向下级页表，
而是直接指向物理地址（RWX不为0b000）。这样物理地址就必须对齐到1G，如果不对齐到1G，就是错误的，会出页错误异常。

地址无关的操作系统需要有两个映射：虚拟地址到物理地址，物理地址自己到物理地址。后者是为了对付教程里提到“尴尬的代码”，
在启动后还未跳转到虚拟地址时，程序仍然暂时可以运行。
这样如果操作系统安装的位置要对齐到4K，就需要两个地址都建立三级页表，其中两个第2级页表可以共用，这样就需要5个4KB
的运行时空间用来存储页表。而且这样只能映射4K的空间，如果要映射更多，应该把虚拟地址的第0级页表写入多个页表项，
需要计算虚拟地址，这样能最多映射到2M的内核启动空间，再多就要加页了。

我实现里的工程设计中，内核是对齐到2M的。这样需要3个4KB页表，如果精心构造第1级页表，最多映射1G的启动空间。
归纳发现，内核的内存对齐越严格，需要的页表就越多，启动空间反而就越小。

所以Sv39的页表项我们要这样理解才合适：

| 54..=63 | 28..=53 | 19..=27 | 10..=18 | 0..=9 |
|:-------:|:-------:|:-------:|:-------:|:-------:|
| 保留 | PPN[2]，26位 | PPN[1]，9位 | PPN[0]，9位 | 一大堆标签 |

在虚拟地址中，存在着VPN[2]、VPN[1]和VPN[0]/VPN[2]编号是2，反而是从satp寄存器里首轮解析的页表。
VPN[0]编号是0，却是最后一次解析的。
三次解析遇到的页表依次可以称作第2级页表、第1级页表、第0级页表。
...
上面的分析我发现，页表解析系统越简单，我们页表占用的空间反而就越小。
比如我同样要对齐到2M，Sv39下只需要解析前两级页表（第2级和第1级），Sv48数字看起来更先进，反而需要解析3级页表（第3、2和1级）。
解析的页表级数越多，就要占用更多的内存空间，虽然后来都是可以回收的。使用较低的Sv39作为启动时的页表模式，是非常好的选择。
当未来的系统里用Sv39启动后，内核加载完毕，开始切换上下文时，就可以考虑使用Sv48或者未来的Sv57等等了。

K210只有6M的片内闪存。如果对齐到2M，SBI先占用前面2M（即使大多数空间都被对齐需求占据了），只剩下4M给内核。
如果内核又占了2M，就几乎没有地方给用户程序和操作系统的其它模块了。这部分就可以考虑做一个跳转，
如果首先是否对齐到2M，如果对齐了，就建立第2、1级Sv39页表以便启动。如果2M对齐满足不了，但是对齐到了4K，
就增加一些启动占用的空间，使用2、1和0三级Sv39页表以便启动。如果4K都对齐不了，那页表就没法创建了，所以必须要求启动地址对齐到4K。
...
发现我昨天的程序在一种例外情况下不适用：就是初始虚拟地址和初始物理地址的VPN[2]相同，
但是VPN[1]或VPN[0]不同的情况。
在2M、4K两种情况下，这两种都会造成页表被覆盖两次，导致要么一个映射不对，要么另一个映射不对。
如果两个都相同，那么初始虚拟地址和初始物理地址相同，只需要把映射更完善的虚拟页初始化放在后面就可以了，
这样就能覆盖物理页的初始化数据。

如果两个地址非常接近（指的是VPN[2]相同，VPN[1]不同，但是VPN[0]相同都为零），也是可以做的，
可以剩下一个页的空间，但是代码相当麻烦而且繁琐，而且这个情况非常非常少，我一开始的确是做了，
效果也可以，但是代码极其难以维护，就[删掉了](https://github.com/luojia65/pcrel-os-lab/commit/604084d97b4b810bff55e9689c3093f372587ec1)。工程设计上建议放弃此类情况。
但是VPN[2]和VPN[1]都相同、VPN[0]不同这种情况是无论如何都做不了的，因为这样就会覆盖虚拟页的部分内容，
导致错误。这次的设计里，最底下的虚拟页应该按照从小到大的顺序初始化，不能因为一个物理页的需求打断，
否则会访问非预期的内存。其实你愿意修这个问题也是能修的，启动后写回去就行了，但是太复杂了，情况太少了，
真的不值得，工程上建议直接设定新的虚拟地址绕开这个问题。

避开这个问题的方法很简单，就是把虚拟地址设置到很高，比如`0xffffffff00000000`，这样就不可能和
初始化过程用到的物理内存地址冲突了。

解决这个问题后，我的任意位置加载的内核就可以根据pc的初始值创建页表了。
我的程序还把使用的页表数量放进`a2`参数里，这样就方便操作系统在内存集创建完毕、新页表使用后，
根据这个参数把启动过程中使用的页回收掉。
...
找到学长[非常棒的笔记](https://github.com/wyfcyx/osnotes/blob/master/book/v3/read-notes.md#lab1)，
对学习有很大的帮助。后面可以抽出时间慢慢看。

的确我想做的SBI实现真的是只支持新的标准，如果支持旧的标准，代码就相当臃肿，而且也不能叫SBI了。
接下来两天到一周的时间，重新给K210适配自己的M级运行时，定制适配与K210的系统。
那么大量包的这种社区开发设计也可以扁平化掉，就是所有的代码全部展开到同一个项目，这样方便很多，
也适合本次实习要求的学习目的。
