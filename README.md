...
宏里面使用了global_asm!，使用`.equ`伪指令定义常量，方便后面的函数使用。
不过这样就需要注意，生成页表的宏必须放在启动指令宏的前面，否则符号会找不到。
还有个问题是可能会不小心用这宏两次，这样错误就很难做了，因为目前是基于静态变量完成的，
出现两次只能提示变量冲突，而不是一些友善的提示。
如果未来能使用`#![my_proc_macro]`，第一就是必定在代码最前面；
第二是不可能出现第二次。

这里涉及到一个没有稳定的点就是[global_asm!](https://github.com/rust-lang/rust/issues/35119)
暂时还需要nightly特性。

如果要使用`#![my_proc_macro]`（内部全局属性宏），思路和目前的过程宏是类似的。
不过这个特性就[还没有稳定](https://github.com/rust-lang/rust/issues/54726)，
好像`cortex-m-rtic`架构也在等这个功能。猜测是因为在跟模块用这个标签，它会解析程序所有的元素，
会让编译和检查过程显著变慢。用过程宏是一个暂时的解决方案。

也许还有更好的解决方案，这也是Rust灵活的地方，需要慢慢探索。

目前制作了[参考实现](https://github.com/luojia65/proc-macro-test)，
这里实现了简单的运行时和初始页表，不使用static而是`.equ`，可以节省储存空间，尤其是必须4K对齐的情况下。
